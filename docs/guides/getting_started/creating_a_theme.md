...
- Updates to [blocks]({% link docs/guides/theme_architecture/blocks/index.md %}) will not update any blocks which have already been added to site pages. However, changes will appear in the block library, which is presented when editing pages. This can lead to having multiple versions of the same block present in the same site. 
