...
Menu templates define their own schema, in a similar way to blocks. All of the attributes defined on a menu's schema are made available via the `Menu` object. The menu is cached by default; to disable caching (for example to show a dynamic cart count), set `cache: false` in the schema — see [Header]({% link docs/site_navigation/header.md %}#header-caching) for details.
